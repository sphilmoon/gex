...
T_cell_markers %>%
    ...
    # look at the most gex in my data
    head(n=100, T_cell_markers)

    # Use literature genes to define major CD4 gene. Top 15.
    # drawing a dot plot
    cd_genes <- c("Cd4", "Cd28", "Ifng", "Tnf", "Foxp3", "Ahr", "Ccr7", "Icos", "Il21", "Lag3", "Lrrc32", "Ccr5")
    ...
    # Th17
    VlnPlot(T_cell_subset, features = c("Il21", "Ifng"), pt.size = 0.3, group.by = "orig.ident")
    FeaturePlot(T_cell_subset, features = c("Il21", "Ifng"), pt.size = 0.3)

    # Tmem
    VlnPlot(T_cell_subset, features = c("Ccr7", "Il21", "Icos"), pt.size = 0.3, group.by = "orig.ident")
    FeaturePlot(T_cell_subset, features = c("Ccr7", "Il21", "Icos"), pt.size = 0.3)

    # Treg
    VlnPlot(T_cell_subset, features = c("Foxp3", "Ahr", "Lrrc32"), pt.size = 0.3, group.by = "orig.ident")
    FeaturePlot(T_cell_subset, features = c("Foxp3", "Ahr", "Lrrc32"), pt.size = 0.3)
